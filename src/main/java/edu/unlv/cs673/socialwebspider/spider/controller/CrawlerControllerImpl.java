...
public class CrawlerControllerImpl implements CrawlerController {
	 ...
	 */
	public void startNewCrawler(String configFolder, int numberOfCrawlers, String storageFolder, int maxDepth, int politenessDelay, String entryPoint, int minBinarySize) {
		try{
			...
			CrawlerImpl crawler = new CrawlerImpl(Patterns.filterOutNonImages, Patterns.patternImage, minBinarySize, crawlDomains, storageFolder);

			controller.start(CrawlerImpl.class, numberOfCrawlers);
		}catch(Exception e){
			System.err.println("An exception occured when running a new crawler.");
			e.printStackTrace();
		}
	}
